...
public class ActorHome {
	...
	public Actor merge(Actor detachedInstance) {
		...
		try {
			Actor result = (Actor) sessionFactory.getCurrentSession().merge(detachedInstance);
	...
	public Actor findById(java.lang.Short id) {
		...
		try {
			Actor instance = (Actor) sessionFactory.getCurrentSession().get("net.tc.sakila.Actor", id);
			if (instance == null) {
	...
	public List findByExample(Actor instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("net.tc.sakila.Actor")
